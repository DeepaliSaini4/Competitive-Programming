...
{ // O(n+r)
    int num = factorial(n);
    int deno = (factorial(n - 1) * factorial(r)) % mod;
...
int nCr5(int n, int r) O(log(mod))
{ // O(n+r)
    int num = fact[n];
    int deno = (fact[n - 1] * fact[r]) % mod;
    return ((num * inverse(deno, mod)) % mod);
}
// EVEN FASTER
int fact[1000100];
...
int precompute()
{
    ...
    incfactorial[1000000] = inverse(fact[1000000]);
...
{ //   O(1)
    int num = fact[n];
    int deno = (incfactorial[n - 1] * incfactorial[r]) % mod;
    return ((num * deno) % mod);
}
signed main()
{
