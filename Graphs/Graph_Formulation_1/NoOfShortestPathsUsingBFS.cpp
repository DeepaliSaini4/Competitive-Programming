//Q--> Finding the number of Shortest path to reach F fron S in the given grid.
#include <bits/stdc++.h>
using namespace std;
        ...
        for(state v:neighbors(node)){
            if(dis[v.F][v.S] == INF){
                ...
                q.push(v);
                   ...
                   num_paths[v.F][v.S] += num_paths[node.F][node.S]; // Adding contribution if can reach from another path as well through the same shortest distance
            }
        }
    }
}
